...
import { ShoppingCart, ArrowLeft, Loader2, PackageX, ExternalLink, Instagram, Twitter, Globe, Mail } from 'lucide-react';
      ...
      <header className="bg-white border-b border-[#1D1D1B]/10 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-10 py-6 flex items-center justify-between">
          ...
          <div className="w-24 flex justify-end">
             {meta?.supportEmail && (
               <a href={`mailto:${meta.supportEmail}`} className="p-2 opacity-40 hover:opacity-100 transition-opacity">
                <Mail className="w-4 h-4" />
               </a>
             )}
             <ShoppingCart className="w-4 h-4 opacity-40 p-2 box-content" />
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-10 pt-12">
        {products.length === 0 ? (
          ...
          <div className="space-y-12">
            {/* Category Filter */}
            <div className="flex items-center gap-8 border-b border-[#1D1D1B]/10 pb-6 overflow-x-auto no-scrollbar">
              {categories.map((cat) => (
                <button
                  key={cat}
      ...
      <footer className="max-w-7xl mx-auto px-10 mt-24 flex flex-col md:flex-row justify-between items-center py-10 border-t border-[#1D1D1B]/10 gap-8">
        <div className="flex flex-col md:flex-row items-center gap-4 md:gap-8 text-[10px] uppercase tracking-editorial font-sans font-bold">
          ...
          {(meta?.website || meta?.instagram || meta?.twitter) && (
            <div className="flex items-center gap-6 border-l border-[#1D1D1B]/10 pl-8 h-4">
              {meta?.website && (
                <a href={meta.website} target="_blank" rel="noreferrer" className="opacity-40 hover:opacity-100 transition-opacity">
                  <Globe className="w-3 h-3" />
                </a>
              )}
              {meta?.instagram && (
                <a href={`https://instagram.com/${meta.instagram.replace('@', '')}`} target="_blank" rel="noreferrer" className="opacity-40 hover:opacity-100 transition-opacity">
                  <Instagram className="w-3 h-3" />
                </a>
              )}
              {meta?.twitter && (
                <a href={`https://twitter.com/${meta.twitter.replace('@', '')}`} target="_blank" rel="noreferrer" className="opacity-40 hover:opacity-100 transition-opacity">
